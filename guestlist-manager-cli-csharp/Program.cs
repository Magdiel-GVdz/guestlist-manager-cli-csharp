...
namespace guestlist_manager_cli_csharp
{
    class Program
    {
        ...
        var connectionString = "mongodb://localhost";
            ...
            MongoHelper database = new MongoHelper(connectionString,
           databaseName);
            ...
            do
            {
                ...
                switch (menuChoice)
                {
                    ...
                    case 1: // Show guest list
                        {
                            var guestsList =
                           database.LoadAllDocuments<GuestModel>(collectionName);
                    ...
                    case 2: // Update guest info (by ID)
                        {
                            ...
                            Guid guestIdGuid;
                            bool isValidGuid = Guid.TryParse(guestId, out
                           guestIdGuid);
                            if (isValidGuid)
                            {
                                var guest =
                               database.LoadDocumentById<GuestModel>(collectionName,
                               guestIdGuid);
                                Console.Write($"\n" +
                                $"Current info\n" +
                               $"Name: {guest.FullName} \n" +
                            ...
                            var fullName = Console.ReadLine();
                               ...
                               !string.IsNullOrWhiteSpace(email))
                                ...
                                database.UpdateDocument<GuestModel>(collectionName, guestIdGuid,
                                guest);
                            }
                            else
                            {
